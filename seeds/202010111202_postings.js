...
exports.seed = (knex) => knex('postings')
  .del()
  .then(async () => {
    ...
    .insert(
      [
        ...R.map(id => Posting(id, (new Date('Feb 1, 2020')).setDate(28), teams[0].id), R.range(0, 20)),
        ...R.map(id => Posting(id, (new Date('Feb 1, 2020')).setMonth(1), teams[0].id), R.range(21, 45)),
      ]
    )
  })
  ...
  .then(async () => {
    ...
    return knex('posting_requirements').insert(
      R.map(i => Requirement(i, i % 3 + postings[0].id), R.range(0, 10)),
    );
  })
  ...
  .then(async () => {
    ...
    return knex('posting_recommended_skills').insert(
      R.map(i => RecommendedSkill(i, i % 3 + postings[0].id), R.range(0, 10)),
    )
  })
  ...
  .then(async () => {
    ...
    return knex('posting_skills_to_be_learned').insert(
      R.map(i => SkillToBeLearned(i, i % 3 + postings[0].id), R.range(0, 10)),
    )
  });
