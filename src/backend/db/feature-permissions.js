...
const keysToIds = async (db, groupKeys) => {
  ...
  if (groupKeys && groupKeys.length > 0) {
    try {
      let query =
        'SELECT id FROM groups WHERE group_key IN (' +
        groupKeys.map((_) => '?').join(',') +
        ');';
      groupIds = (await db.raw(query, [...groupKeys])).rows.map(
        (row) => row.id,
      );
    } catch (e) {
      console.log(e);
...
const appendKeysToIds = async (db, groupIds) => {
  ...
  if (groupIds && groupIds.length > 0) {
    try {
      ...
      let query =
        'SELECT id, group_key FROM groups WHERE id IN (' +
        groupIds.map((_) => '?').join(',') +
        ');';
...
const getGroupKeys = async (gAdminClient, userId) => {
  try {
    ...
    console.log(
      'Error: Google Admin API groups.list() call returned an error: ',
      err,
...
const getAllowedActions = (db) => async (userId, accessToken) => {
  try {
    ...
    if (groupIds && groupIds.length > 0) {
      let query =
        'SELECT DISTINCT feature_name FROM feature_permissions WHERE group_id IN (' +
        groupIds.map((_) => '?').join(',') +
        ');';
      allowedActions = (await db.raw(query, [...groupIds])).rows.map(
...
const updateUserGroups = (db) => async (userId, groupIds, accessToken) => {
  ...
  try {
    ...
    await db.raw(`DELETE FROM group_users WHERE user_id = :userId;`, {
      userId: userId,
    });
      ...
      db.transaction(function (trx) {
        db('group_users')
          ...
          .then(function () {
            ...
            const memberIds = resultList.data.members.map(
              (member) => member.id,
            );
            ...
            if (memberIds && memberIds.length > 0) {
              return db('group_users')
                .insert(
                  memberIds.map((m_Id) => {
                    return { user_id: m_Id, group_id: groupId };
                  }),
                )
                .transacting(trx);
            }
        ...
        .then(function () {
          console.log(
            'Successfully updated Google Groups membership info for group: ' +
              groupKey,
          );
        })
